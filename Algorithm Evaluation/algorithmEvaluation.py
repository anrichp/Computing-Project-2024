        ...
        if filename.endswith(".txt"):
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as file:
                    ...
                    logging.info(
                        f"Processed file: {filename} in {readability_level} level"
                    )
            except Exception as e:
...
df["Human Evaluation Numeric"] = df["Human Evaluation"].map(
    {"Elementary": 1, "Intermediate": 2, "Advanced": 3}
)
...
logging.info(
    f"Readability assessments and correlation matrix exported to readability_assessments_with_correlation.xlsx"
)
